...
function setGlobalView(view) {
  ...
  // Favorites shares the view toggle too
...
function renderFavorites() {
  ...
  const isGrid = S.view.live !== 'list'; // share view toggle with live
  ...
  if (isGrid) {
    ...
    if (list) list.innerHTML = items.map(f =>
      listItem(f.item, f.type)
    ).join('');
  }
}

// Wire fav-type tabs on first load (they're static HTML, not re-rendered)
...
function liveCard(ch) {
  ...
  return `
    <div class="card" onclick="openDetail(${JSON.stringify(ch).replace(/"/g,'&quot;')}, 'live')">
      ...
      <div class="card-poster-placeholder wide" style="overflow:hidden;">
...
function mediaCard(item, type) {
  ...
  return `
    <div class="card" onclick="openDetail(${itemJ}, '${type}')">
      ${isFav ? '<div class="card-fav active">★</div>' : '<div class="card-fav">☆</div>'}
      <div class="card-poster-placeholder" style="overflow:hidden;">
...
function listItem(item, type) {
  ...
  return `
    <div class="list-item" onclick="openDetail(${itemJ}, '${type}')">
      <div class="list-thumb" style="width:48px;height:48px;border-radius:var(--radius-xs);overflow:hidden;flex-shrink:0;background:var(--bg-4);position:relative;">
